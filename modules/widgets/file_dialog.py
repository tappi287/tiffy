...
class FileDialog:
    ...
    def open_existing_file(cls, parent=None,
        ...
        if file and Path(file).exists():
            TiffySettings.app['current_path'] = Path(file).parent.as_posix()
            TiffySettings.add_recent_file(file, file_key)
    ...
    def open_existing_directory(cls, parent=None, directory: Union[Path, str]=None,) -> Union[str, None]:
        ...
        if directory and Path(directory).exists():
            TiffySettings.app['current_path'] = Path(directory).as_posix()
    ...
    def _get_current_path(d) -> Path:
        ...
        __c = Path(TiffySettings.app['current_path'])
        # Fallback path USERPROFILE path or current directory '.'
        __fallback = Path(os.getenv('USERPROFILE', '.'))

        if not d or not Path(d).exists():
            if TiffySettings.app['current_path'] not in ['', '.'] and __c.exists():
